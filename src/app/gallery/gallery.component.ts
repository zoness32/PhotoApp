...
export class GalleryComponent implements OnInit, OnDestroy, OnChanges {
    @Input() flexBorderSize = 3;
    @Input() flexImageSize = 7;
    @Input() galleryName = 'Lily';
    @Input() providedMetadataUri: string = undefined;
  ...
  public gallery: any[] = [];
  public imageDataStaticPath = 'assets/img/gallery/';
  public imageDataCompletePath = '';
  public dataFileName = 'data.json';
  public images: any;
  public minimalQualityCategory = 'preview_xxs';
  public viewerSubscription: Subscription;

    @HostListener('window:scroll', ['$event']) triggerCycle() {
        this.scaleGallery();
    }
    ...
    constructor(public imageService: ImageService, public httpClient: HttpClient, public changeDetectorRef: ChangeDetectorRef) {
    }
    ...
    private calcImageMargin() {
        ...
        return Math.round(Math.max(1, this.flexBorderSize * ratio));
    }
    ...
    private calcIdealHeight() {
        return this.getGalleryWidth() / (80 / this.flexImageSize) + 100;
    }
